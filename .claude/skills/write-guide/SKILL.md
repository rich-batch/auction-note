---
name: write-guide
description: 경매·공매 SEO 가이드 글(content/guide)을 pipeline/guide/keywords.csv 기준으로 새로 쓰거나(공식 자료 조사·경쟁 글 비교·중복 판단 → 출처 링크·표·요약 박스가 들어간 초안 → ELI5 커버 이미지 생성 → AI 흔적 셀프 리뷰), 기존 글을 고쳐 쓰거나, 발행 전 8항목 품질 점검(시각 요소 포함)을 하거나, 수치 갱신 점검을 한다. "글 써줘", "priority N 작성", "다음 글", "고쳐 써줘", "검수해줘", "발행 전 점검", "갱신 점검", "커버 이미지 다시 만들어줘" 요청 시 사용. 특정 경매 물건(사건번호) 분석 글은 analyze-case 스킬이 담당한다.
...
글이 아니라 **사이트 자체**(필수 페이지, 애드센스 심사 준비, 30일 일정표)를 다룰 때는 `site-readiness` 스킬을, 특정 경매 물건 분석 글은 `analyze-case` 스킬을 쓴다. 저장소 구조와 공통 규칙은 루트 `CLAUDE.md`.

경로(글 하나 = slug 하나):

| 무엇 | 경로 |
|---|---|
| 글 | `content/guide/<slug>/index.md` (커버·본문 이미지도 같은 폴더) |
| 조사 노트 | `pipeline/guide/research/<slug>.md` |
| ELI5 스펙 | `pipeline/guide/research/<slug>.eli5.json` (+ 렌더링된 `.svg`) |
| 키워드 목록 | `pipeline/guide/keywords.csv` |
| 검사 | `node tools/check/guide.mjs content/guide/<slug>/index.md` |
...
1. **대상 고르기.** 인자 없음 → 다음 1편(조사·비교가 무거워 기본은 1편). 숫자 N → N편(최대 3편 권장). "1순위"/키워드 → 그 한 편.
   ```bash
   node .claude/skills/write-guide/render-prompt.mjs --next 1 --type info
   node .claude/skills/write-guide/render-prompt.mjs --priority 1
   node .claude/skills/write-guide/render-prompt.mjs --slug <slug>
   node .claude/skills/write-guide/render-prompt.mjs --list
   ```
   `SKIPPED:`는 같은 슬러그 파일이 이미 있어 건너뛴 행이다. 덮어쓰지 않는다(고치려면 고쳐쓰기 모드).
2. **0단계(조사·경쟁 글 비교·중복 판단).** `=== JOB` 블록의 0단계를 끝까지 하고 `pipeline/guide/research/<slug>.md`를 저장한다. 지시문이 "멈춰라"라고 한 경우(기존 글과 80% 이상 중복, 차별점 없음)는 **글을 쓰지 않고** 5단계 보고로 넘어간다.
3. **1단계(글쓰기)** 후 저장 → **1-2단계(ELI5 커버 이미지)**: `pipeline/guide/research/<slug>.eli5.json` 작성 → 렌더링 → PNG를 Read로 열어 눈으로 확인 → **2단계(셀프 리뷰)**로 다시 읽고 고쳐 저장한다.
   ```bash
...
4. **검사.** FAIL은 PASS까지 고친다. "확인 권장"은 하나씩 읽고 고칠지 판단한다(목록 비율이 높으면 설명 문장으로 풀어 쓰기).
   ```bash
   node tools/check/guide.mjs content/guide/<slug>/index.md
   ```
...
`tools/check/guide.mjs` 결과와 글·조사 노트를 읽고 항목별로 **통과 / 보완 필요**를 판정해 표로 준다. 보완 필요면 무엇을 어떻게 고칠지 한 줄. 사실관계 수치는 맞다고 단정하지 말고 "사람이 공식 자료로 확인 필요"로 표시한다.

| # | 항목 | 판정 기준 |
|---|---|---|
| ① | 출처 표기 | 수치마다 공식 출처 링크, 참고 자료 목록, 조사 노트에 원문 인용 |
| ② | 면책 문구 | 사이트 템플릿이 자동으로 붙인다(`layouts/_partials/disclaimer.html`) — 본문에 중복 삽입만 없으면 통과 |
...
1. `pipeline/guide/research/<slug>.eli5.json` 작성(형식·규칙은 템플릿 1-2단계): title, analogy(한 줄 비유), steps 3~4개(label·desc), takeaway, footer
2. 렌더링:
   ```bash
   node tools/render-eli5.mjs pipeline/guide/research/<slug>.eli5.json
   ```
   - 출력: `pipeline/guide/research/<slug>.eli5.svg`(원본), `content/guide/<slug>/eli5-overview.png`
...
스킬이 남긴 `[이미지 필요: <파일명>.png | alt: … | 캡션: … | 구조: …]`를 사람이 교체한다.

1. 구조대로 순수 도형·화살표로 그린다(캔바 등, 또는 공개 통계 수치로 직접 그린 그래프). 캡처·검색 이미지·기관 로고 금지
2. 가로 1200px 안팎으로 줄여 글 폴더 `content/guide/<slug>/<파일명>.png`에 저장:
   ```bash
   sips -Z 1200 content/guide/<slug>/<파일명>.png
   ```
3. 자리표시자 줄을 아래 두 줄로 바꾼다(이미지 줄 바로 아래가 캡션):
   ...
   *핵심 한 문장 (수치가 있으면 출처 링크와 YYYY년 M월 D일 기준)*
   ```
4. `pipeline/guide/research/<slug>.md`에 `## 이미지` 섹션을 만들고 `- <파일명>.png: 만든 방법, 사용한 데이터와 출처`를 적는다(검사 스크립트가 확인)
...
1. 점검 대상 뽑기:
   ```bash
   grep -l '^volatile: true' content/guide/*/index.md
   node tools/check/guide.mjs content/guide/<slug>/index.md --facts
   ```
...
**FAIL:** front matter(title·author "rich-batch"·date·lastmod·draft·volatile·categories·tags·description), 영문 슬러그 폴더명, 미래 날짜, 제목 연도, description "~정리합니다", `#` 제목, 서론 2~3문단, 기준일 문장 = lastmod, 소제목 4~6개, 표, FAQ 3~5개, 참고 자료, 고지 / 수치 줄 출처 링크, 허용 도메인(go.kr·korea.kr·bok.or.kr·kfb.or.kr·khug.or.kr·kamco.or.kr·onbid.co.kr·fss.or.kr), 참고 자료 누락 URL, 조사 노트에 없는 URL / 조사 노트의 중복 판단·경쟁 글 비교·차별점 섹션 / 예고형·일반론 서론, 공허한 맺음, "은행마다 다릅니다"류, 가상 인물 의심("제 지인", "김 씨는"), 투자 권유 / `draft: false`인데 `[경험 추가 필요]` 남음

시각 요소: `cover`(image = `eli5-overview.png`, 글 폴더에 파일 존재, alt·caption 빈 말 아님, 캡션 수치엔 링크), `pipeline/guide/research/<slug>.eli5.json` 존재, 서론 직후 `> **핵심 요약**` 박스(1~3줄), 박스 최대 2개, 수치 표 아래 출처 줄에 기준일, 본문 이미지(외부 URL 금지, 글 폴더에 파일 존재·파일명만으로 참조, alt, 바로 아래 `*캡션*`, 영문 파일명, 500KB 이하, 조사 노트 `## 이미지`에 기록), `[이미지 필요]` 최대 1개·필드 완비·`draft: false`면 남으면 안 됨

**확인 권장(WARN):** 제목 22자 초과, 제목에 "완벽 정리·총정리·모든 것"류 상투어, 소제목 "N가지", 경험 자리 0곳, 본문 목록·표 비율 50% 초과, 추상 형용사 4회 이상, 표·박스·목록·이미지 없이 문단 6개 이상 연속(글자 벽), 표가 글 분량에 비해 많음, 이미지 200KB 초과·가로 1500px 초과
...
## keywords.csv (`pipeline/guide/keywords.csv`)

열: `priority,keyword,slug,category,seed,intent,suggested_title,notes,my_experience`

- `slug`: URL. **발행 후 바꾸지 않는다.**
- `suggested_title`: 검색 의도·소재 파악용 참고일 뿐, front matter의 `title`로 그대로 쓰지 않는다. 실제 title은 20자 안팎으로 압축한다(자세한 규칙은 템플릿의 title 항목 참고).
...
새 성격은 `templates/<type>.md`를 추가한다. 자리: `{keyword}` `{slug}` `{category}` `{seed}` `{intent}` `{suggested_title}` `{notes}` `{priority}` `{my_experience}` `{date}`(KST 오늘) `{created}`(기존 글이면 원래 date) `{research_path}` `{existing_posts}` `{related_keywords}`(같은 카테고리 다른 키워드와 작성 여부). 구조가 다르면 `tools/check/guide.mjs` 규칙도 나눠야 한다.
...
| archetype과 형식이 다름 | `archetypes/guide.md`가 가이드 글 front matter 모양(YAML). `default.md`(TOML)는 소개·연락처 같은 일반 페이지용 |
| 이미지가 사이트에서 깨짐 | 이미지는 `static/`이 아니라 글 폴더(`content/guide/<slug>/`)에 두고 글에서는 파일명만 쓴다(`![…](cost-flow.png)`) |
| 다른 글 링크가 깨짐 | `/guide/<slug>/`처럼 직접 쓰면 `/auction-note/` 하위 경로가 빠진다. `{{< relref "/guide/<slug>" >}}`를 쓴다 |
